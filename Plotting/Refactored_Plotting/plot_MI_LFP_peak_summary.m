...
% Also saves separate 15-panel LFP and MI figures, ordered E15 -> E1.
%
...
all_electrodes = arrayfun(@(e) sprintf('E%d', e), 15:-1:1, 'UniformOutput', false);
...
[lfp_time, std_lfp_by_electrode, dvt_lfp_by_electrode] = collect_lfp_traces(participants, basefold, datatype, ...
    ...
    corrected, srate, baseline, stim_onset, all_electrodes, expected_timepoints);

[mi_time, mi_by_electrode, sig_by_electrode, peak_times, elec_names_rev, participants_used] = collect_mi_data( ...
    participants, basefold, condition, activity_tag, all_electrodes, stim_onset, expected_timepoints);
...
%% All-electrode figures (reuse the traces already loaded above)
fig_lfp = plot_all_electrode_traces(all_electrodes, lfp_time, ...
    std_lfp_by_electrode, dvt_lfp_by_electrode, struct(), 'LFP', ...
    stim_onset_plot_ms, line_width, axis_width);
fig_mi = plot_all_electrode_traces(all_electrodes, mi_time, ...
    mi_by_electrode, struct(), sig_by_electrode, 'MI', ...
    stim_onset_plot_ms, line_width, axis_width);

extra_figures = [fig_lfp fig_mi];
extra_stems = {[save_stem '_all_electrodes_LFP'], [save_stem '_all_electrodes_MI']};
for f = 1:numel(extra_figures)
    output_stem = fullfile(out_dir, extra_stems{f});
    saveas(extra_figures(f), [output_stem '.fig']);
    print(extra_figures(f), [output_stem '.png'], '-dpng', '-r300');
    print(extra_figures(f), [output_stem '.svg'], '-dsvg', '-painters');
end
fprintf('Saved all-electrode LFP and MI figures to:\n%s\n', out_dir);

end

function fig = plot_all_electrode_traces(electrodes, time_ms, traces, deviant_traces, ...
    sig_traces, plot_type, stim_onset_ms, line_width, axis_width)
% Each row of a trace array is one fly's mean; SEM is across flies.
fig = figure('Color', 'w', 'Position', [100 100 1500 850], ...
    'Name', ['All electrodes: ' plot_type]);
layout = tiledlayout(fig, 3, 5, 'TileSpacing', 'compact', 'Padding', 'compact');
is_lfp = strcmp(plot_type, 'LFP');
legend_added = false;
for e = 1:numel(electrodes)
    ax = nexttile(layout, e);
    elec = electrodes{e};
    hold(ax, 'on');
    if is_lfp
        h_std = plot_mean_sem(time_ms, traces.(elec), [0.20 0.20 0.20], line_width);
        h_dvt = plot_mean_sem(time_ms, deviant_traces.(elec), [0.00 0.45 0.25], line_width);
        if ~legend_added && isgraphics(h_std) && isgraphics(h_dvt)
            legend(ax, [h_std h_dvt], {'Standard', 'Deviant'}, ...
                'Box', 'off', 'Location', 'best');
            legend_added = true;
        end
    else
        plot_mean_sem(time_ms, traces.(elec), [0.10 0.25 0.75], line_width);
        limits = ylim(ax);
        ylim(ax, [0 max(limits(2), eps)]);
        add_all_fly_sig_bar(time_ms, sig_traces.(elec), [0.65 0.00 0.65], line_width);
    end
    xline(ax, stim_onset_ms, '--k', 'LineWidth', 0.3);
    if ~isempty(time_ms)
        xlim(ax, [time_ms(1) time_ms(end)]);
    end
    title(ax, sprintf('%s %s', elec, plot_type));
    format_axes(ax, axis_width);
end
xlabel(layout, 'Time from stimulus onset (ms)');
if is_lfp
    ylabel(layout, 'LFP amplitude (a.u.)');
else
    ylabel(layout, 'MI (bits)');
end
end
