...
function [Temp_figure_all, Front_figure_all] = plot_highest(basefold, participant, participants, condition, HighAF, times, xlimits, ylimits_CoI, ylimit_MI, xticks_CoI, yticks_CoI, yticks_MI, x_labels, y_labels_CoI, y_labels_MI, climits, climits_mask)
...
for participanti = 1:length(participants)
    ...
    tempFFi = HighAF.(char(participants(participanti))).temp.data;
    tempFFm = logical(HighAF.(char(participants(participanti))).temp.sigMask);
    tempFFmr = logical(HighAF.(char(participants(participanti))).temp.sigMask > 0);
    tempFFms = logical(HighAF.(char(participants(participanti))).temp.sigMask < 0);
    tempFFmi1 = HighAF.(char(participants(participanti))).temp.MI;

    frontFFi = HighAF.(char(participants(participanti))).front.data;
    frontFFm = logical(HighAF.(char(participants(participanti))).front.sigMask);
    frontFFmr = logical(HighAF.(char(participants(participanti))).front.sigMask > 0);
    frontFFms = logical(HighAF.(char(participants(participanti))).front.sigMask < 0);
    frontFFmi1 = HighAF.(char(participants(participanti))).front.MI;
