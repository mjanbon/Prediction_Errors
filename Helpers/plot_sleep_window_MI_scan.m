...
if ~isempty(subject)
    for i = 1:numel(summaries)
        ...
        draw_panels(s.window_centers,s.sleep_mean,[], ...
...
draw_panels(100*grid,group.mean_sleep,group.sem_sleep,channels,heading, ...
    'Relative window position (0 = first, 100 = last; not elapsed time)',stem);
save([stem,'.mat'],'group','-v7.3');
end

function draw_panels(x,sleep,semSleep,channels,heading,xlabelText,stem)
