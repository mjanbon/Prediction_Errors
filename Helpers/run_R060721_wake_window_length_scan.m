function run_R060721_wake_window_length_scan()
% run_R060721_wake_window_length_scan
%
% Scan wake-only MI across windows of increasing length for multiple
% channels.
% For each window length, this helper slides a window across the wake
...
%     E12, E10, E7, E4, and E1, each with the full-wake mean as a dotted
...
try
    ...
    max_random_samples_per_length = 10; % cap for the random-sampling baseline.

    [basefold, datatype, ~, ~, ~, ~, ~, srate, activity_tag_1, ...
        deviant_group_number_1, standard_group_number_1, activity_tag_2, ...
        ~, ~, corrected, ...
        ~, baseline, ~, ~, ~] = Max_get_comparison_param(0, 0);
    ...
    if any(channel_list > dvt_wake.nbchan)
        error('One or more requested channels exceed available channels (%d).', dvt_wake.nbchan);
    end

    % Wake-only comparison baseline is computed exactly as in the existing helper.
    full_wake_pair = baseline_normalize_pair(dvt_wake, std_wake, baseline);

    % Determine scan range.
    wake_trial_count = std_wake.trials;
    if isempty(min_window_len), min_window_len = max(50, floor(wake_trial_count / 20)); end
    if isempty(max_window_len), max_window_len = wake_trial_count; end
    ...
    outdir = fullfile(repoRoot, 'Data', 'MI_Data', 'tmp_R060721', 'wake_window_length_scan');
    if ~exist(outdir, 'dir')
        mkdir(outdir);
    end
    ...
    channel_results = repmat(struct( ...
        ...
        'results', [], ...
        'slide_mean_means', [], ...
        'random_mean_means', []), 1, numel(channel_list));

    for iCh = 1:numel(channel_list)
        summary_channel = channel_list(iCh);
        chan_name = sprintf('E%d', summary_channel);

        wake_data_full = extract_channel_trials(full_wake_pair, summary_channel);
        stim_full = build_stim(size(wake_data_full.dev, 1), size(wake_data_full.std, 1));

        fprintf('Computing MI on full wake data for %s...\n', chan_name);
        [full_wake_MI, full_wake_sig] = cnm_MI_stimtime([wake_data_full.dev; wake_data_full.std], stim_full, 0);
        full_wake_mean = mean(full_wake_MI(:), 'omitnan');
        ...
        for iLen = 1:numel(window_lengths)
            ...
            max_start = wake_trial_count - win_len + 1;
            if max_start < 1
                error('Window length %d exceeds available wake trials (%d).', win_len, wake_trial_count);
            end
            ...
            for w = 1:nStarts
                ...
                wake_window = trim_window_pair(dvt_wake, std_wake, start_idx, stop_idx);
                wake_window = baseline_normalize_pair(wake_window.dev, wake_window.std, baseline);
                wake_data = extract_channel_trials(wake_window, summary_channel);
                stim = build_stim(size(wake_data.dev, 1), size(wake_data.std, 1));

                [wake_MI, ~] = cnm_MI_stimtime([wake_data.dev; wake_data.std], stim, 0);
                window_MI{w} = wake_MI;
                window_means(w) = mean(wake_MI(:), 'omitnan');
            end

            fprintf('    %s length %d/%d: random samples=%d\n', chan_name, iLen, numel(window_lengths), nRandom);
            for r = 1:nRandom
                rand_idx = sort(randperm(wake_trial_count, win_len));
                random_indices{r} = rand_idx;

                random_window = subset_window_pair(dvt_wake, std_wake, rand_idx);
        ...
        channel_results(iCh).full_wake_MI = full_wake_MI;
        channel_results(iCh).full_wake_sigMask = full_wake_sig;
        channel_results(iCh).full_wake_mean = full_wake_mean;
    ...
    summary.full_wake_means = arrayfun(@(r) r.full_wake_mean, channel_results);

    save(fullfile(outdir, sprintf('%s_%s_wake_window_length_scan.mat', subject, condition_tag)), ...
        'summary', '-v7.3');
    ...
    for iCh = 1:numel(channel_list)
        ...
        yline(channel_results(iCh).full_wake_mean, 'k:', 'LineWidth', 2);
        xlabel('Wake window length (trials)');
        ylabel('Mean MI across windows');
        title(sprintf('%s', chan_name));
        legend({'slide\_mean\_mean', 'random sample mean', 'full wake mean'}, 'Location', 'best');
        grid on;
    end

    sgtitle(sprintf('%s %s wake window-length scan', subject, condition_tag));

    saveas(gcf, fullfile(outdir, sprintf('%s_%s_wake_window_length_scan.png', subject, condition_tag)));
    saveas(gcf, fullfile(outdir, sprintf('%s_%s_wake_window_length_scan.fig', subject, condition_tag)));

    fprintf('\nSaved summary to %s\n', fullfile(outdir, sprintf('%s_%s_wake_window_length_scan.mat', subject, condition_tag)));
    fprintf('Done.\n');
